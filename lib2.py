...
async def create_jwt(region: str) -> Tuple[str, str, str]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            decoded = decode_protobuf(response_content, FreeFire_pb2.LoginRes)
            message_data = json.loads(json_format.MessageToJson(decoded))
            token = message_data.get("token", "0")
            region = message_data.get("lockRegion", "0")
            serverUrl = message_data.get("serverUrl", "0")
            return f"Bearer {token}", region, serverUrl
        except Exception as e:
            print("❌ Error decoding LoginRes:", str(e))
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            if "application/octet-stream" not in response.headers.get("Content-Type", ""):
                print("❌ Response không phải protobuf:")
                print("Status code:", response.status_code)
                print("Content-Type:", response.headers.get("Content-Type"))
                print("Body:", response.text)
                return {
                    "error": "Invalid response",
                    "detail": "Server did not return protobuf data."
                }

            parsed_data = decode_protobuf(response_content, AccountPersonalShow_pb2.AccountPersonalShowInfo)
            message = json.loads(json_format.MessageToJson(parsed_data))
            return message

        except message.DecodeError as e:
            print("❌ DecodeError khi ParseFromString:", str(e))
            print("Raw content:", response_content)
            return {
